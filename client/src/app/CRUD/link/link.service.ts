...
export class LinkService {
   ...
   getFiltrado(columna: string, tipoFiltro: string, filtro: string): Promise<Link[]> {
      return this.http.get(this.urlBase+'/leer_filtrado' + '?columna=' + columna + '&tipo_filtro=' + tipoFiltro + '&filtro=' + filtro).toPromise().then(response=>response.json() as Link[]).catch(this.handleError);
   }
   ...
   get(id: number): Promise<Link> {
      ...
      return this.http.get(url).toPromise().then(response=>(response.json() as Link[])[0]).catch(this.handleError);
   }

   remove(id: number): Promise<boolean> {
      const url = `${this.urlBase+'/borrar'}?id=${id}`;
      return this.http.get(url).toPromise().then(response=>response.json() as Link).catch(this.handleError);
   }

   create(entidadTransporte: Link): Promise<boolean> {
   ...
   update(entidadTransporte: Link): Promise<boolean> {
