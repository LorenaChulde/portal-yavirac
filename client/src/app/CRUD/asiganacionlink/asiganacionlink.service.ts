...
export class AsiganacionLinkService {
    ...
    private urlBase2 = environment.apiUrl + 'asiganacionlink_link';
    ...
    getAsiganacionLinkLink(): Promise<AsiganacionLinkLink[]> {
        return this.http.get(this.urlBase2 + '/consultar').toPromise().then(response => response.json() as AsiganacionLinkLink[]).catch(this.handleError);
    }
    ...
    getFiltrado(columna: string, tipoFiltro: string, filtro: string): Promise<AsiganacionLink[]> {
        return this.http.get(this.urlBase + '/leer_filtrado' + '?columna=' + columna + '&tipo_filtro=' + tipoFiltro + '&filtro=' + filtro).toPromise().then(response => response.json() as AsiganacionLink[]).catch(this.handleError);
    }
    ...
    get(id: number): Promise<AsiganacionLink> {
        ...
        return this.http.get(url).toPromise().then(response => (response.json() as AsiganacionLink[])[0]).catch(this.handleError);
    }

    remove(id: number): Promise<boolean> {
        const url = `${this.urlBase + '/borrar'}?id=${id}`;
        return this.http.get(url).toPromise().then(response => response.json() as AsiganacionLink).catch(this.handleError);
    }
