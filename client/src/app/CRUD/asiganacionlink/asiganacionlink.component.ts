...
@Component({
    selector: 'app-asiganacionlink',
    templateUrl: './asiganacionlink.component.html',
    styleUrls: ['./asiganacionlink.component.scss']
})
export class AsiganacionLinkComponent implements OnInit {
    busy: Promise<any>;
    entidades: AsiganacionLink[];
    entidadesAsiganacionLinkPagina: AsiganacionLinkPagina[];
    entidadesAsignacionLinkDLink: AsignacionLinkDLink[];
    ...
    constructor(
        ...
        private dataService: AsiganacionLinkService,
    ...
    getAll(): void {
        this.busy = this.dataService
            ...
            .then(entidadesRecuperadas => {
                ...
                if (
                    entidadesRecuperadas == null ||
                    entidadesRecuperadas.length === 0
                ) {
    ...
    getAsignacionLinkDLink(): void {
        this.busy = this.dataService
            .getAsignacionLinkDLink()
            .then(entidadesRecuperadas => {
                this.entidadesAsignacionLinkDLink = entidadesRecuperadas;
                console.log (this.entidadesAsignacionLinkDLink);
                if (
                    entidadesRecuperadas == null ||
                    entidadesRecuperadas.length === 0
                ) {
    ...
    isValid(entidadPorEvaluar: AsiganacionLink): boolean {
        return true;
    }
    ...
    aceptar(): void {
        ...
        if (
            this.entidadSeleccionada.id === undefined ||
    ...
    crearEntidad(): AsiganacionLink {
        const nuevoAsiganacionLink = new AsiganacionLink();
        nuevoAsiganacionLink.id = 0;
        return nuevoAsiganacionLink;
    }

    add(entidadNueva: AsiganacionLink): void {
        this.busy = this.dataService
            .create(entidadNueva)
            .then(respuesta => {
                if (respuesta) {
    ...
    update(entidadParaActualizar: AsiganacionLink): void {
        this.busy = this.dataService
            .update(entidadParaActualizar)
            .then(respuesta => {
                if (respuesta) {
                    this.toastr.success(
    ...
    delete(entidadParaBorrar: AsiganacionLink): void {
        this.busy = this.dataService
    ...
    refresh(): void {
        ...
        this.entidades = AsiganacionLink[0];
        this.entidadSeleccionada = this.crearEntidad();
    }
