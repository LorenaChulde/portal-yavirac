...
import { AsiganacionLinkLink } from '../../entidades/especifico/AsiganacionLinkLink';
...
export class AsiganacionLinkComponent implements OnInit {
  ...
  entidadesAsiganacionLinkLink: AsiganacionLinkLink[];
  ...
  constructor(public toastr: ToastsManager, vcr: ViewContainerRef, private dataService: AsiganacionLinkService, private modalService: NgbModal) {
  ...
  getAsiganacionLinkLink(): void {
    this.busy = this.dataService
      .getAsiganacionLinkLink()
      .then(entidadesRecuperadas => {
        this.entidadesAsiganacionLinkLink = entidadesRecuperadas;
        if (entidadesRecuperadas == null || entidadesRecuperadas.length === 0) {
          this.toastr.success('¡No hay datos!', 'Consulta');
        } else {
          this.toastr.success('La consulta fue exitosa', 'Consulta');
