...
export class ImagenComponent implements OnInit {
    ...
    busy: Promise<any>;
    entidades: Imagen[];
    entidadesImagen: Imagen[];
    entidadSeleccionada: Imagen;
    pagina: 1;
    tamanoPagina: 20;
    paginaActual: number;
    paginaUltima: number;
    ...
    imagenType: string;
    imagenFile: Blob;
    ImagenService: string;

    constructor(public toastr: ToastsManager, vcr: ViewContainerRef, private dataService: ImagenService, private modalService: NgbModal) {
    ...
    insertarImagen(entidadNueva: Imagen): void {
       ...
       entidadNueva.tipoArchivo = this.imagenType;
        ...
        this.busy = this.dataService.insertarImagen(entidadNueva)
            .then(respuesta => {
                if (respuesta) {
    ...
    getImagen(): void {
        this.busy = this.dataService
            .getImagen()
            .then(entidadesRecuperadas => {
                this.entidadesImagen = entidadesRecuperadas;
                if (entidadesRecuperadas == null || entidadesRecuperadas.length === 0) {
                    this.toastr.success('¡No hay datos!', 'Consulta');
                } else {
                    this.toastr.success('La consulta fue exitosa', 'Consulta');
    ...
    isValid(entidadPorEvaluar: Imagen): boolean {
        return true;
    }

    aceptar(): void {
        this.insertarImagen(this.entidadSeleccionada);
        this.cerrarVentanaEdicion();
    }

    crearEntidad(): Imagen {
        const nuevoImagen = new Imagen();
        nuevoImagen.id = 0;
        return nuevoImagen;
    }

    add(entidadNueva: Imagen): void {
    ...
    update(entidadParaActualizar: Imagen): void {
    ...
    delete(entidadParaBorrar: Imagen): void {
    ...
    onSelect(entidadActual: Imagen): void {
